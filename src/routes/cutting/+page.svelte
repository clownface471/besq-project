<script lang="ts">
  ...
  let recentScans = [
    { id: 1, lot: "KPCP-2309-A01", timeStart: "10:30", timeEnd: "11:15", status: "Proses", pic: "Tono W." },
    { id: 2, lot: "KPCP-2309-A02", timeStart: "10:15", timeEnd: "11:00", status: "Selesai", pic: "Tono W." },
    { id: 3, lot: "KPCP-2309-B05", timeStart: "09:45", timeEnd: "10:30", status: "Selesai", pic: "Tono W." },
  ];
  ...
  const maxChartValue = Math.max(...dailyCompounds.map(d => d.actual), ...dailyCompounds.map(d => d.target), 6);
...
<div class="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12 relative selection:bg-indigo-100 selection:text-indigo-800">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8 flex justify-between items-center">
            ...
            <button on:click={handleLogout}
                ...
                <div class="flex items-center gap-2 relative z-10">
                    ...
                    <svg class="w-5 h-5 md:w-4 md:h-4 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                </div>
            </button>
        </div>
    </div>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 md:-mt-24 relative z-10 space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-100 overflow-visible group hover:shadow-lg transition-all duration-500 mx-auto w-full">
      <div class="p-6 md:p-10">
        <div class="flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start">
          <div class="relative shrink-0 -mt-16 md:-mt-20 group-hover:-translate-y-2 transition-transform duration-500">
            <div class="w-28 h-28 md:w-44 md:h-44 rounded-2xl overflow-hidden border-4 md:border-[6px] border-white shadow-lg ring-1 ring-slate-100">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 lg:p-8 flex flex-col h-full">
            ...
            <div class="md:hidden -mx-6 px-6 mb-4">
                <div class="flex overflow-x-auto gap-3 pb-2 scrollbar-hide">
                    {#each dailyCompounds as day}
                      {#if day.short !== 'Min'}
                        <div class="group flex flex-col items-center justify-end shrink-0 w-16 relative">
                            <div class="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-xs py-1 px-2 rounded pointer-events-none z-10 whitespace-nowrap">
                                {day.target}/{day.actual}
                            </div>
                            <div class="relative w-10 h-40 bg-slate-50 rounded-t-lg overflow-hidden flex items-end shadow-sm border border-slate-100">
                                <div 
                            ...
                            <div class="mt-2 text-center">
                                ...
                                <p class="text-[10px] text-slate-400 font-mono mt-0.5">{day.actual}L</p>
                            </div>
                        </div>
                      {/if}
                    {/each}
            ...
            <div class="hidden md:flex flex-1 items-end justify-between gap-2 md:gap-4 h-52 md:h-64 pb-2 border-b border-slate-100">
                {#each dailyCompounds as day}
                  {#if day.short !== 'Min'} <div class="group flex flex-col items-center justify-end h-full w-full relative">
                        ...
                        <div class="mt-3 text-center">
                            ...
                            <p class="text-[10px] text-slate-400 font-mono mt-0.5">{day.actual} Lot</p>
                        </div>
                    </div>
                  {/if}
                {/each}
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 lg:p-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500">Daftar lot yang baru saja discan dan diproses. Klik untuk scan lot baru.</p>
            </div>
        </div>
        ...
        <div class="md:hidden space-y-3">
            ...
            <div class="p-4 border border-slate-100 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer" on:click={handleRowClick}>
                <div class="flex justify-between items-start mb-3">
                    <span class="font-mono font-bold text-indigo-600">{scan.lot}</span>
                    <span class={`px-2 py-1 rounded-full text-xs font-bold border ${getStatusColor(scan.status)}`}>
                        {scan.status}
                    </span>
                </div>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-slate-700">{scan.timeStart} WIB</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Waktu Selesai:</span>
                        <span class="font-medium text-slate-700">{scan.timeEnd} WIB</span>
                    </div>
                    <div class="flex justify-between">
            ...
            {#if recentScans.length === 0}
                <div class="py-8 text-center text-slate-400">Belum ada data scan hari ini.</div>
            {/if}
        </div>
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm">
                    {#each recentScans as scan}
                    <tr class="group hover:bg-slate-50 transition-colors border-b last:border-0 border-slate-50 cursor-pointer" on:click={handleRowClick}>
                        <td class="py-4 font-mono font-bold text-slate-700 group-hover:text-indigo-600 transition-colors">
                            {scan.lot}
                        </td>
                        ...
                        <td class="py-4 text-slate-600 font-medium">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-slate-200 text-[10px] flex items-center justify-center font-bold text-slate-500">{scan.pic.charAt(0)}</div>
                                {scan.pic}
                            </div>
                        </td>
                        <td class="py-4 text-right">
                            <span class={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(scan.status)}`}>
                                {scan.status}
                            </span>
                        </td>
                    </tr>
                    {/each}
                    {#if recentScans.length === 0}
                        <tr><td colspan="5" class="py-8 text-center text-slate-400">Belum ada data scan hari ini.</td></tr>
                    {/if}
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-4 pt-2 border-t border-slate-50 text-center md:text-left">
             <a href="/history" class="text-sm text-indigo-600 font-semibold hover:text-indigo-800 transition-colors">Lihat Riwayat Lengkap &rarr;</a>
        </div>
    </div>
