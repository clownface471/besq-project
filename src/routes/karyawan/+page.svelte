...
<div class="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12 relative">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
    <!-- Header -->
    <div
      class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6"
    >
      <div
              ...
              {:else}
                {#each users as user (user.id)}
                  <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border {getRoleBadgeColor(
                          user.role
                        )}"
                      >
                        {user.role}
