<script lang="ts">
	...
	// Role guard on mount
	onMount(() => {
		if (!$auth.user || ($auth.user.role !== 'pressing' && $auth.user.role !== 'admin')) {
			alert('⚠️ Unauthorized: Only Pressing or Admin users can access this page');
			goto('/');
			return;
		}
	...
	async function fetchDailyLogs() {
		try {
			isLoading = true;
			const response = await fetch('/api/pressing/today', {
				headers: {
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			if (!response.ok) {
				throw new Error('Failed to create pressing entry');
			}

			successMessage = '✓ Pressing entry recorded successfully';
		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
			<div>
				<h1 class="text-2xl md:text-3xl font-bold text-slate-800">
					⚙️ Pressing Department
				</h1>
				<p class="text-sm text-slate-500 mt-1">Production Input & Daily Log</p>
			</div>
			<div class="text-right">
				<p class="text-sm text-slate-600 font-medium">
					{$auth.user?.name || 'User'} • <span class="capitalize bg-amber-50 text-amber-700 px-2 py-1 rounded inline-block">{$auth.user?.role}</span>
				</p>
					...
					<form onsubmit={handleSubmit} class="space-y-4">
						...
						<div>
							...
							<input
								...
								class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition"
								disabled={isLoading}
							/>
						</div>
						...
						<button
							type="submit"
							disabled={isLoading}
							class="w-full bg-amber-600 hover:bg-amber-700 disabled:bg-slate-400 text-white font-semibold py-2 px-4 rounded-lg transition flex items-center justify-center gap-2"
						>
							{#if isLoading}
				...
				<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
					...
					<div class="grid grid-cols-3 gap-4 p-6 border-b border-slate-200 bg-slate-50">
						<div class="text-center">
							<div class="text-2xl font-bold text-amber-600">{totalQuantity}</div>
						...
						<div class="text-center">
							<div class="text-2xl font-bold text-orange-600">{totalProduction}</div>
								...
								<tbody>
									{#each dailyLogs as log (log.id)}
										<tr class="border-b border-slate-100 hover:bg-slate-50 transition">
											...
											<td class="px-6 py-4 text-sm text-center font-bold text-orange-600">
												{log.total}
											</td>
										</tr>
									{/each}
